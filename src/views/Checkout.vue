<template>
    <div class="flex flex-col items-center border-b bg-white py-4 sm:flex-row sm:px-10 lg:px-20 xl:px-32">
        <a href="#" class="text-2xl font-bold text-gray-800">ChanzStore.</a>
    ...
    <div class="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
        <div class="px-4 pt-8">
            <p class="text-xl font-medium">Metode Pembayaran</p>
            ...
            <div class="mt-8 space-y-3 rounded-lg border bg-white px-2 py-4 sm:px-6">
                <div v-for="cart in getCart" :key="cart.cart_id"
                    ...
                    <img class="m-2 h-24 w-28 rounded-md border object-cover object-center"
                        src="https://img.freepik.com/free-vector/ecommerce-web-page-concept-illustration_114360-8204.jpg?w=740&t=st=1693374100~exp=1693374700~hmac=fa680fc32daaa8a6a6cf52b97c20e26a37082c180994e66e4b75f274b599a05f"
                        alt="" />
            ...
            <div class="">
                ...
                <div class="mt-6 border-t border-b py-2">
                    <div class="flex items-center justify-between">
                        ...
                        <p class="font-semibold text-gray-900">{{ (totalHarga()).toLocaleString('id-ID',{style: 'currency',currency:'IDR'}) }}</p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p class="text-sm font-medium text-gray-900">Shipping</p>
                        <p class="font-semibold text-gray-900">{{(ongkir).toLocaleString('id-ID',{style: 'currency',currency:'IDR'}) }}</p>
                    </div>
                </div>
                <div class="mt-6 flex items-center justify-between">
                    <p class="text-sm font-medium text-gray-900">Total</p>
                    <p class="text-2xl font-semibold text-gray-900">{{ (totalHarga() + ongkir).toLocaleString('id-ID',{style: 'currency',currency:'IDR'}) }}</p>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        // total price
        totalHarga() {
            this.total = this.getCart.reduce((acc, cart) => {
        ...
        async performCheckout() {
            // Collect the cart IDs to be checked out
            const cartItemIds = this.getCart.map(cart => cart.cart_id);
            // Get user address
